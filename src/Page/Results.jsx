...
function Results() {
  ...
  const loadStudents = async () => {
    try {
      ...
      setStudents(res.data.student || res.data || []);
    } catch {
      showAlert("Failed to load students", "error");
    }
  };
  ...
  const generateResult = async () => {
    ...
    if ((classLevel === 11 || classLevel === 12) && !stream)
      return showAlert("Stream required for 11 & 12", "error");
    ...
    const subjectResult = subjects.map(s => {
      total += s.marks;
      if (s.marks < 33) fail = true;
      return {
        ...s,
        grade: gradeFromMarks(s.marks),
        status: s.marks >= 33 ? "PASS" : "FAIL",
      };
    });
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6">
      {alert && (
        <div className={`fixed top-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-xl text-black ${
          alert.type === "error" ? "bg-red-400" : "bg-green-400"
        }`}>
          {alert.msg}
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <Select label="Institute" value={instituteId} onChange={setInstituteId}>
          {institutes.map(i => (
            <option key={i._id} value={i._id}>{i.name}</option>
          ))}
        </Select>

        <Select label="Class" value={classLevel} onChange={handleClass}>
          {[...Array(12)].map((_, i) => (
      ...
      {(classLevel === 11 || classLevel === 12) && (
        <Select label="Stream" value={stream} onChange={handleStream}>
          {Object.keys(streams).map(s => (
